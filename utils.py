""" This file contains misc functions, including logger, distributed helper etc. """
import os 
import time 
import subprocess 
import datetime 
...
""" The functions for distributed models and processes end here. """


def makedirs(dirs): 
    """ Make dirs according to input dirs. """
    def makedir(dir): 
        if not os.path.exists(dir): 
            os.makedirs(dir) 
    if isinstance(dirs, tuple) or isinstance(dirs, list): 
        for dir in dirs: 
            makedir(dir) 
    else: 
        makedir(dirs) 
...
def sorted_angle_dir(list1: str): 
    indices = sorted(
        range(len(list1)), 
        key=lambda index: int(list1[index].split('e')[-1]) 
    )
    list2 = [list1[index] for index in indices] 
    return list2 
